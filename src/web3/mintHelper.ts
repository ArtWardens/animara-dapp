...
import { CandyGuard, CandyMachine, GuardGroup, DefaultGuardSet, DefaultGuardSetMintArgs, getMerkleRoot, route, getMerkleProof, safeFetchAllowListProofFromSeeds, mintV2 } from "@metaplex-foundation/mpl-candy-machine";
import { DigitalAssetWithToken, TokenStandard } from "@metaplex-foundation/mpl-token-metadata";
import { some, Umi, transactionBuilder, publicKey, TransactionBuilder, none, AddressLookupTableInput, Transaction, Signer, BlockhashWithExpiryBlockHeight } from "@metaplex-foundation/umi";
...
export const mintArgsBuilder = (
  ...
  // skip allow list
  mintArgs.allowList = some({ merkleRoot: getMerkleRoot([]) });
  ...
  if (guards.nftPayment.__option === "Some") {
    ...
    if (!nft) {
      ...
      let tokenStandard = TokenStandard.NonFungible;
      let ruleSet: PublicKey | undefined = undefined;
      if (nft.metadata.tokenStandard.__option === "Some") {
        if (
          nft.metadata.tokenStandard.value ===
          TokenStandard.ProgrammableNonFungible
      ...
      tx2 = tx2.add(
        route(umi, {
          ...
          routeArgs: {
            path: "proof",
            merkleRoot: getMerkleRoot([]),
            merkleProof: getMerkleProof([], publicKey(umi.identity)),
          },
        })
      );
    }
