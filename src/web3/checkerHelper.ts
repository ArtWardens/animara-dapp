...
export const allocationChecker = async (
  ...
  try {
    ...
    if (mintCounter) {
      ...
      toast("Allocation Guard not Initialized!");
...
export const calculateMintable = (
  ...
  if (!process.env.REACT_APP__MAXMINTAMOUNT) return mintableAmount;
  let maxmintamount = 0;
  try {
    maxmintamount = Number(process.env.REACT_APP__MAXMINTAMOUNT)
  } catch (e){
    console.error('process.env.REACT_APP__MAXMINTAMOUNT is not a number!', e)
