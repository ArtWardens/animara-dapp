...
import { handleLogout } from '../firebase/auth.ts';
import { useUserStore } from '../store/store.ts';

const NavbarV2 = () => {
  const [othersDropdownVisible, setOthersDropdownVisible] = useState(false);
  ...
  return (
    <>
      ...
      <nav
        className={`mobile-navbar fixed overflow-hidden ${mobileNavVisible ? '' : 'translate-x-full'} h-full w-full bg-black bg-opacity-75 backdrop-blur-sm z-50 flex justify-end transition-all`}
      >
