...
import { getUserUpgradeLocation, useUserLocationLoading } from "../../sagaStore/slices/userSlice.js";
...
const UpgradeDetailsModal = ({ upgrade, onClose }) => {
  ...
  const [isMaxLevelReached, setIsMaxLevelReached] = useState(false); 
  ...
  useEffect(() => {
    if (hasStartedUpgrade && !isUserLocationLoading) {
      if (upgrade.level === upgrade.maxLevel - 1) {
        setIsMaxLevelReached(true); 
        setIsExploredSuccessfully(false); 
      } else {
        setIsExploredSuccessfully(true); 
        setIsMaxLevelReached(false); 
      }
    }
  ...
  const handleUpgrade = () => {
    ...
    dispatch(getUserUpgradeLocation(upgrade.locationId));
  };
  ...
  return (
    ...
    >
      {isUserLocationLoading && (
        <div className="absolute inset-0 flex justify-center items-center rounded-lg w-full z-[210]">
      ...
      {!isUserLocationLoading && (isExploredSuccessfully || isMaxLevelReached) ? (
        <div className="absolute inset-0 flex justify-center items-center rounded-lg w-full z-[210]" >
        ...
        !isUserLocationLoading && (
          ...
          >
            {/* Close Button */}
            <button className="absolute top-20 right-20 text-white text-4xl" onClick={onClose} disabled={isUserLocationLoading}>
              &times;
            </button>
